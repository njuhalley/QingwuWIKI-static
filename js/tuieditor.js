...
function addImageBlobHook(fileOrBlob, callback, source) {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            layerIndex = layer.load(1, { shade: [0.1, '#fff'] // 0.1 透明度的白色背景
            });
        },
        error: function () {
        ...
        success: function (data) {
            ...
            callback(data.url, imageFile.name);
            console.log(data.url);
            if (data.errcode !== 0) {
                layer.msg(data.message);
                var value = '![](' + data.url + ')';
                window.editor.insertText(value);
            }
            return false
...
$(function () {
    ...
    // window.test_editor = editormd("docEditor", {
    //     width: "100%",
    //     height: "100%",
    ...
    $("#sidebar").jstree({
        ...
        $(window.editor).ready(function () {
            // this.hideToolbar();
            // 监听按键
            window.editor.addCommand("markdown", {
                name: 'save_doc',
                keyMap: ['CTRL+S'],
                exec: function exec() {
                    saveDocument(false)
            ...
            window.editor.addCommand("wysiwyg", {
                name: 'save_doc',
                keyMap: ['CTRL+S'],
                exec: function exec() {
                    saveDocument(false)
    ...
    $(window.editor).on("paste", function () {
        console.log("test")
    });
    window.editor.addHook("addImageBlobHook",addImageBlobHook);
    window.editor.addHook("change", resetEditorChanged);
    // window.editor.eventManager.listen('addImageBlobHook', );
});